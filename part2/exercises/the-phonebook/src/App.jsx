...
const App = () => {
  ...
  const showInfoMessage = (text, duration) => showMessage(text, 'info', duration)

  const showErrorMessage = (text, duration) => showMessage(text, 'error', duration)
      ...
      if (person.name.toLowerCase() === name.toLowerCase()) {
        if(confirm(`${name} is already added to phonebook, replace the old number with a new one?`)) {
          ...
          personsService
            ...
            .then(returnedPerson => {
              showInfoMessage(`Updated ${returnedPerson.name} number`, 5000)
  ...
  const handleDeletePerson = (person) => {
    if (confirm(`Do you really want to delete ${person.name}?`)) {
      personsService
        ...
        .then(returnedPerson => {
          showInfoMessage(`Deleted ${returnedPerson.name}`, 5000)
          setPersons(persons.filter(p => p.id !== returnedPerson.id))
        })
        .catch(error => {
          showErrorMessage(`Information of ${person.name} has already been removed`, 5000)
        })
    } 
  }
